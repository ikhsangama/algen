    ...
    <h2>Simulasi Pencarian Rute Terpendek</h2>
		...
		<div class="card">
			<div class="container">
				<?php
				$nomor=0;
				$sql	= 'SELECT * FROM variabel_algen ORDER BY id_variabel';
				$query = mysqli_query($dbconnect,$sql);
				if(mysqli_num_rows($query) <> 0){
						while($data = mysqli_fetch_array($query)){ $nomor++; ?>
							Prob Crossover: <?php echo $data['prob_crossover']?> <br>
							Prob Mutasi: <?php echo $data['prob_mutasi']?> <br>
							Jml Kromosom Awal: <?php echo $data['jml_krom_awal']?> <br>
							Jml Generasi:<?php echo $data['jml_generasi']?>
						<?php }
				}
				?>
			</div>
