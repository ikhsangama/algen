...
//---------------------------Inisialisasi-----------------------------------------
 ...
 $startakhir = ["Rektorat","Rektorat"];
 $kunjungan = ["FEB", "FPP", "FSM", "FK", "WP", "FT", "FPIK", "FPsi"];
 $jumlahkromosomawal = 25;
 $generasi = 150;
 $probmutasi = 0.3;
 $probcrossover = 0.3;
 $jumlahpercobaan = 20;
...
for($q=0; $q<$jumlahpercobaan; $q++){
	...
	foreach($copyfitnessterbaik as $x => $value) {
	    //echo "<br>Kromosom ke-" . ($x+1) . " fitness=" . $value;
		$rankingterbaik[$m] = $x;
		$nilaifitness[$m]= $value;
		$m++;
	}
	$iterasihasilterbaik[$q] = $rankingterbaik[0];
	$fitnessterbaikgenerasi[$q] = $nilaifitness[0];
	
	//echo "<br>Solusi Terbaik adalah iterasi ke-".$iterasihasilterbaik."";
	//$hasilakhir = $solusi[$iterasihasilterbaik];
	//echo " dengan fitness = ".$fitnessterbaikgenerasi[$jumlahpercobaan];
...
echo "<table border='1'>
			<tr>
				<th>Percobaan</th>
				<th>Nilai Fitness</th>
				<th>Solusi Terbaik</th>
			</tr>
			";
for($w=0; $w<$jumlahpercobaan; $w++){
	$rata2fitness = $rata2fitness + $fitnessterbaikgenerasi[$w];
		echo "<tr align='center'>
				<td>".($w+1)."</td>
				<td>".$fitnessterbaikgenerasi[$w]."</td>
				<td>".$iterasihasilterbaik[$w]."</td>
			</tr>
		 
	";
}
$rata2fitness = $rata2fitness/$jumlahpercobaan;
echo "	<tr>
			<td>Rata-rata</td>
			<td>".$rata2fitness."</td>
			<td></td>
